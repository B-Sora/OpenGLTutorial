...
struct Update
{
	void operator()(double delta) {
		...
		if (interval <= 0) {
			const std::uniform_real_distribution<> randInterval(1.0,1.0);
			...
			for (int i = randAddingCount(game.Rand()); i > 0; --i) {
				...
				if (Entity::Entity* p = game.AddEntity(EntityGroupId_Enemy,pos, "Toroid", "Res/Toroid.bmp",UpdateToroid())) {
					//p->Velocity(glm::vec3(pos.x < 0 ? 1.0f : -1.0, 0, -4.0f));
					p->Velocity(glm::vec3(pos.x < 0 ? 4.0f : -4.0, 0, -16.0f));
...
* 
* @param data Uniform blockのサイズ
* @param data Uniform blockに転送するデータへのポイント
* 
* @return 作成したもの UBO
*/
