...
#define SetVertexAttribPointer(index, cls, mbr) SetVertexAttribPointerI( \
	...
	/**
	* Vertex Arrray Objectを作成する
	*
	* @param vbo VAOに関連付けられる VBO
	* @param ibo VAOに関連付けられる IBO
	*
	* @return 作成したVAO
	*/
	...
	* メッシュを描画する
	*
	* @param buffer描画に使用するメッシュバッファへのポインタ
	*/
	...
	*/
	bool Buffer::LoadMeshFromFile(const char* filename) {
		...
		for (TemporaryMesh& e : loader.meshList) {
			...
			const size_t beginMatreial = endMaterial - e.materialList.size();
			meshList.insert(std::make_pair(
				e.name, std::make_shared<Impl>(e.name,beginMatreial, endMaterial)));
		}
		return true;
	}
